    ...
    for epoch in range(start_epoch, config.training.epochs + 1):
        ...
        if is_main_process():
            ...
            if config.training.save_last_checkpoint:
                save_checkpoint(output_dir / "last.pt", model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, config=config.to_dict(), metrics=val_metrics)
            ...
            if config.training.save_best_checkpoint and (best_metric is None or _is_better_checkpoint_metric(metric_name, candidate, best_metric)):
                ...
                save_checkpoint(output_dir / "best.pt", model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, config=config.to_dict(), metrics=val_metrics)
        barrier()
    destroy_distributed()
    return {
    ...
    *,
    ...
    for step_index, batch in enumerate(loader, start=1):
        ...
        if training and save_every_steps > 0 and step_index % save_every_steps == 0 and is_main_process():
            save_checkpoint(output_dir / "last_step.pt", model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, step=step_index, config=config.to_dict(), metrics=metrics)
